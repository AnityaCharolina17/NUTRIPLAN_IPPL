...
/**
 * Case-Based Reasoning (CBR) Menu Result
 */
export interface CBRMenuCase {
  id: string;
  menuName: string;
  description?: string;
  calories?: number;
...
/**
 * CBR Menu Generation Result
 */
export interface CBRGenerationResult {
  success: boolean;
  baseIngredient: {
    id: string;
    name: string;
 ...
 */
export async function generateMenusFromBaseIngredient(
  ingredientId: string,
  limit: number = 3
): Promise<CBRGenerationResult> {
  try {
 ...
 * @returns CBR generation result with retrieved cases
 */
export async function generateMenusByIngredientName(
 ...
 */
export async function getAllMenuCases() {
  try {
    const cases = await prisma.menuCase.findMany({
      include: {
        baseIngredient: {
 ...
 */
export async function getMenuCaseStatistics() {
  try {
    // Get total cases
    const totalCases = await prisma.menuCase.count();

    // Get cases per ingredient
    const casesPerIngredient = await prisma.menuCase.groupBy({
      by: ["baseIngredientId"],
      _count: true,
    });
 ...
 */
export async function validateMenuCaseAvailability(ingredientId: string) {
  try {
    ...
    const caseCount = await prisma.menuCase.count({
