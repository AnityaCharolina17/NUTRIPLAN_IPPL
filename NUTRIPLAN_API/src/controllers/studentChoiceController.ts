 ...
 */
export const getChoiceStatus = async (req: AuthRequest, res: Response) => {
  try {
    const now = new Date();
 ...
 */
export const getMyChoices = async (req: AuthRequest, res: Response) => {
  try {
 ...
 */
export const updateMyChoice = async (req: AuthRequest, res: Response) => {
  try {
    const studentId = req.user?.userId;
    ...
    // Upsert pilihan
    const existing = await prisma.studentMenuChoice.findFirst({
      where: {
        studentId,
    ...
    let result;
    if (existing) {
      result = await prisma.studentMenuChoice.update({
        where: { id: existing.id },
        data: { choice, isAutoAssigned: false },
      });
    } else {
      result = await prisma.studentMenuChoice.create({
        data: {
          studentId,
          weekStart: new Date(weekStart),
          day,
