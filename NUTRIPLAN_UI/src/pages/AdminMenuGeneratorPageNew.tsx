...
import { Sparkles, Calendar, ChefHat, Save, Loader2, Edit2 } from 'lucide-react';
...
import { MenuSuggestion } from '../lib/aiMenuGenerator';
import { generateMenuSuggestions as generateMenuSuggestionsAPI } from '../lib/backendMenuGenerator';
...
export function AdminMenuGeneratorPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Prefer backend AI generator; fallback to local if fails
      let generated: MenuSuggestion[] = [];
      try {
        generated = await generateMenuSuggestionsAPI(mainIngredient);
      } catch {
        const { generateMenuSuggestions } = await import('../lib/aiMenuGenerator');
        generated = await generateMenuSuggestions(mainIngredient);
      }
  ...
  const handleSaveMenu = async () => {
    ...
    // Create a new menu item (local for UI)
    ...
    // Update weekly schedule (local)
    updateWeeklySchedule(selectedDay, newMenuId, mainIngredient);

    // Also persist to backend weekly menu (minimal one-day save)
    try {
      // Derive weekStart/weekEnd from current schedule list (take first and last dates)
      const dates = weeklySchedule.map(s => new Date(s.date));
      ...
      await api.post('/menus/save', {
        ...
        items: [
          {
            day: selectedDay,
            mainDish: selectedMenu.name,
            sideDish: selectedMenu.ingredients[1] || 'Tahu',
            vegetable: 'Sayur',
            fruit: 'Buah',
            drink: 'Air Putih',
            calories: selectedMenu.nutritionFacts.calories,
            protein: `${selectedMenu.nutritionFacts.protein}g`,
            carbs: `${selectedMenu.nutritionFacts.carbs}g`,
            fat: `${selectedMenu.nutritionFacts.fat}g`,
            portionCount: 150,
            ingredients: (selectedMenu.ingredients || []).map((ing) => ({
              ingredient: ing,
              quantity: undefined,
              unit: undefined,
            })),
      ...
      console.warn('Backend save menu failed:', err?.response?.data || err?.message);
    }
