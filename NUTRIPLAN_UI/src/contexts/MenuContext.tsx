...
export function MenuProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    api.get('/api/menus/current', { validateStatus: () => true })
      .then((res) => {
        if (res.status >= 400 || !res.data?.success || !res.data?.menu?.items) {
          console.warn('No active menu from API');
          return;
        }
      ...
      .catch((err) => {
        console.warn('Failed to fetch current menu', err?.response?.data?.message || err?.message || err);
      });
    ...
    const fetchHistory = async () => {
      try {
        ...
        for (let weeksBack = 1; weeksBack <= 4; weeksBack++) {
          ...
          const res = await api.get(`/api/menus/week/${monday.toISOString()}`, { validateStatus: () => true });
          if (res.status < 400 && res.data?.success && res.data?.menu?.items) {
          ...
          // If no data, just skip without throwing
        }
